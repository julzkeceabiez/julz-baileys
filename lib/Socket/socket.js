...
const makeSocket = (config) => {
    ...
    const onMessageReceived = (data) => {
        noise.decodeFrame(data, frame => {
            ...
            // PATCH (stabilitas): seluruh proses dispatch frame dibungkus try/catch.
            // Sebelumnya, kalau ADA SATU SAJA listener (baik di lib ini maupun di
            // kode bot pemakainya, mis. handler pesan) yang melempar error secara
            // synchronous saat merespons sebuah frame, error itu jadi uncaught
            // exception di sini dan bisa menjatuhkan SELURUH proses Node — bukan
            // cuma koneksi WA-nya. Ini salah satu penyebab paling umum bot "tiba-tiba
            // mati sendiri" tanpa pesan error yang jelas. Sekarang error semacam itu
            // ditangkap & diteruskan ke onUnexpectedError (yang sudah ada mekanisme
            // pemulihannya untuk kasus umum seperti bad-mac/rate-limit) alih-alih
            // mematikan proses.
            try {
                ...
                onUnexpectedError(err, 'processing frame');
            }
        });
    };
    const end = (error) => {
